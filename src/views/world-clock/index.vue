...
<template>
  <div class="min-h-screen bg-bg-deep text-text-primary font-body flex flex-col">
    ...
    <div v-if="showPicker" class="max-w-7xl w-full mx-auto px-4 sm:px-6 mb-6">
      <div class="border border-border-default bg-bg-surface p-4">
        ...
        <div class="max-h-48 overflow-y-auto flex flex-wrap gap-2 pr-1">
          ...
          <span v-if="filteredCities.length === 0" class="text-text-secondary text-sm py-2"
            >Không tìm thấy thành phố nào.</span
          >
        </div>

        <p class="text-text-secondary text-xs mt-3">
          Click để bật/tắt · Đang hiện {{ activeCities.length }} / {{ ALL_CITIES.length }} thành phố
        </p>
      </div>
    </div>
      ...
      >
        ...
        >
          ...
          <button
            ...
            class="absolute top-1.5 right-1.5 w-5 h-5 flex items-center justify-center text-text-secondary opacity-0 group-hover:opacity-100 transition hover:text-accent-coral text-base leading-none"
          >
            ×
          </button>
          ...
          <div class="text-center">
            <img
              ...
              :alt="city.country"
            />
            <div class="font-display font-bold text-text-primary text-xs leading-tight">
              {{ city.name }}
            </div>
          ...
          <div class="relative w-20 h-20">
            <svg class="w-full h-full" viewBox="0 0 120 120">
              <circle
                ...
                cy="60"
                r="56"
                :fill="isNight(city.timezone) ? '#0f172a' : '#1e293b'"
                stroke="currentColor"
                stroke-width="1.5"
              ...
              <circle
                ...
                fill="none"
                stroke-width="1"
                :stroke="isNight(city.timezone) ? '#6366f1' : '#f97316'"
                opacity="0.25"
              />
              <line
                v-for="i in 12"
            ...
            <div class="absolute top-0.5 right-0.5 text-[10px]">
              {{ isNight(city.timezone) ? '🌙' : '☀️' }}
            </div>
          </div>
          ...
          <div class="text-center">
            <div
              class="font-display font-bold text-lg tabular-nums leading-tight text-text-primary"
            >
              {{ getTimeInZone(city.timezone) }}
            </div>
            <div class="text-text-secondary text-[10px] mt-0.5">
              {{ getDateInZone(city.timezone) }}
            </div>
            <div
              class="text-[10px] font-semibold mt-0.5"
              :class="isNight(city.timezone) ? 'text-indigo-400' : 'text-orange-400'"
            >
    ...
    <!-- Footer -->
    <footer
      class="border-t border-border-default py-4 px-6 text-center text-text-secondary text-xs"
    >
      Made by
      <a
        href="https://kingcongstudio.com"
        target="_blank"
        rel="noopener"
        class="text-accent-coral hover:underline ml-1"
        >KingCong Studio</a
      >
    </footer>
  </div>
