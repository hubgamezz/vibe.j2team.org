  ...
  <div class="page">
    <div class="wrap">
      ...
      <div v-if="paused && screen === 'playing'" class="overlay">
        ...
        <button class="g-btn g-btn-ghost" @click="restartGame" style="margin-top: 8px">
          ↩ Chơi lại
        </button>
        <RouterLink class="home-link" to="/"
          ><svg
...
function drawSky(): void {
  ...
  g.addColorStop(0, '#100800')
  g.addColorStop(0.3, '#2a1200')
  g.addColorStop(0.65, '#4a2200')
  g.addColorStop(1, '#1a0e00')
  c.fillStyle = g
  c.fillRect(0, 0, W, H)
  ...
  c.beginPath()
  c.arc(W * 0.75, 42, 18, 0, Math.PI * 2)
  c.fill()
  c.fillStyle = 'rgba(255,250,220,0.5)'
  c.beginPath()
  c.arc(W * 0.75, 42, 12, 0, Math.PI * 2)
  c.fill()
}
...
function drawGround(): void {
  ...
  c.fillStyle = '#160b00'
  c.beginPath()
  c.ellipse(W / 2, H - 30, 200, 25, 0, 0, Math.PI * 2)
  c.fill()
  c.strokeStyle = 'rgba(80,40,0,0.4)'
  c.lineWidth = 0.5
  for (let i = -3; i <= 3; i++) {
    c.beginPath()
    c.ellipse(W / 2 + i * 60, H - 30, 25, 10, 0, 0, Math.PI * 2)
    c.stroke()
  }
}
...
function drawBuddha(): void {
  ...
  c.fillStyle = ag
  c.fillRect(0, 0, W, H)
  c.strokeStyle = 'rgba(255,180,40,0.45)'
  c.lineWidth = 1.5
  c.beginPath()
  c.arc(bx, by - 105, 42, 0, Math.PI * 2)
  c.stroke()
  c.strokeStyle = 'rgba(255,180,40,0.12)'
  c.lineWidth = 10
  c.beginPath()
  c.arc(bx, by - 105, 48, 0, Math.PI * 2)
  c.stroke()
  c.fillStyle = 'rgba(20,10,0,0.9)'
  c.beginPath()
  c.ellipse(bx, by + 5, 52, 12, 0, 0, Math.PI * 2)
  c.fill()
  c.beginPath()
  c.moveTo(bx - 50, by + 5)
  c.lineTo(bx + 50, by + 5)
  c.lineTo(bx + 38, by - 10)
  c.lineTo(bx - 38, by - 10)
  c.closePath()
  c.fill()
  c.strokeStyle = 'rgba(255,180,40,0.25)'
  c.lineWidth = 0.8
  for (let i = 0; i < 8; i++) {
    const a = (i / 8) * Math.PI * 2
    c.beginPath()
    c.ellipse(bx + Math.cos(a) * 30, by + 3 + Math.sin(a) * 6, 8, 4, a, 0, Math.PI * 2)
    c.stroke()
  }
  ...
  c.fill()
  c.beginPath()
  c.ellipse(bx, by, 50, 18, 0, 0, Math.PI * 2)
  c.fill()
  c.beginPath()
  c.ellipse(bx, by - 25, 30, 10, 0, 0, Math.PI * 2)
  c.fill()
  c.beginPath()
  c.ellipse(bx, by - 118, 10, 8, 0, 0, Math.PI * 2)
  c.fill()
  c.beginPath()
  c.ellipse(bx, by - 133, 20, 23, 0, 0, Math.PI * 2)
  c.fill()
  c.beginPath()
  c.ellipse(bx, by - 158, 9, 12, 0, 0, Math.PI * 2)
  c.fill()
  c.strokeStyle = 'rgba(255,180,40,0.2)'
  c.lineWidth = 0.8
  c.beginPath()
  c.ellipse(bx, by - 133, 20, 23, 0, 0, Math.PI * 2)
  c.stroke()
  const t = frame * 0.012
  for (let i = 0; i < 2; i++) {
    c.strokeStyle = `rgba(255,220,150,${0.06 + i * 0.02})`
    c.lineWidth = 1.5
    c.beginPath()
    c.moveTo(bx - 15 + i * 30, by + 10)
    for (let j = 1; j <= 18; j++)
...
function drawTree(): void {
  ...
  c.closePath()
  c.fill()
  c.strokeStyle = 'rgba(60,30,0,0.5)'
  c.lineWidth = 1
  for (let i = 0; i < 4; i++) {
    c.beginPath()
  ...
  ;(
    ...
    c.lineWidth = 7
    c.strokeStyle = '#0e0600'
    c.beginPath()
    c.moveTo(bx, by)
    c.quadraticCurveTo(
      bx + Math.cos(ang) * len * 0.5,
      by + Math.sin(ang - Math.PI / 2) * len * 0.4,
      bx + Math.cos(ang) * len,
      by + Math.sin(ang - Math.PI / 2) * len,
    )
    c.stroke()
  })
  ;(
  ...
  ;[
    { x: tx, y: ty - 320, rx: 108, ry: 65 },
    { x: tx - 85, y: ty - 295, rx: 75, ry: 50 },
    { x: tx + 85, y: ty - 285, rx: 80, ry: 52 },
    { x: tx - 130, y: ty - 265, rx: 60, ry: 42 },
    { x: tx + 115, y: ty - 260, rx: 65, ry: 45 },
    { x: tx - 45, y: ty - 350, rx: 70, ry: 48 },
    { x: tx + 40, y: ty - 340, rx: 75, ry: 52 },
    { x: tx - 15, y: ty - 285, rx: 85, ry: 55 },
    { x: tx + 155, y: ty - 240, rx: 50, ry: 38 },
  ].forEach((cl, i) => {
    c.fillStyle =
      i % 3 === 0 ? 'rgba(12,38,4,0.97)' : i % 3 === 1 ? 'rgba(18,52,6,0.94)' : 'rgba(22,60,8,0.92)'
    c.beginPath()
    c.ellipse(cl.x, cl.y, cl.rx, cl.ry, i * 0.35 - 0.6, 0, Math.PI * 2)
    c.fill()
  })
  c.restore()
}
...
function drawLeaf(l: Leaf): void {
  ...
  c.beginPath()
  c.ellipse(2, 2, l.w / 2, l.h / 2, 0, 0, Math.PI * 2)
  c.fill()
  const lg = c.createLinearGradient(-l.w / 2, 0, l.w / 2, 0)
  lg.addColorStop(0, l.col)
  lg.addColorStop(0.5, l.col2)
  lg.addColorStop(1, l.col)
  c.fillStyle = lg
  c.beginPath()
  c.moveTo(-l.w / 2, 0)
  ...
  for (let i = -2; i <= 2; i++) {
    ...
    c.beginPath()
    c.moveTo(0, 0)
    c.lineTo(i * (l.w / 6), i > 0 ? -l.h / 2 + 2 : l.h / 2)
    c.stroke()
  }
  c.restore()
}
...
function drawBowl(): void {
  ...
  c.beginPath()
  c.ellipse(x, y + h / 2 + 6, w / 2 + 6, 7, 0, 0, Math.PI * 2)
  c.fill()
  const bg = c.createLinearGradient(x - w / 2, 0, x + w / 2, 0)
  bg.addColorStop(0, '#6b3d1a')
  bg.addColorStop(0.3, '#b8713a')
  bg.addColorStop(0.65, '#cc845a')
  bg.addColorStop(1, '#6b3d1a')
  c.fillStyle = bg
  c.beginPath()
  c.moveTo(x - w / 2, y - 1)
  ...
  rg.addColorStop(0, '#7a4820')
  rg.addColorStop(0.5, '#d4956a')
  rg.addColorStop(1, '#7a4820')
  c.fillStyle = rg
  c.beginPath()
  c.ellipse(x, y, w / 2, h / 4, 0, 0, Math.PI * 2)
  c.fill()
  c.fillStyle = 'rgba(25,10,0,0.7)'
  c.beginPath()
  c.ellipse(x, y, w / 2 - 4, h / 4 - 2, 0, 0, Math.PI * 2)
  c.fill()
  c.strokeStyle = 'rgba(255,200,80,0.45)'
  c.lineWidth = 0.8
  c.beginPath()
  c.arc(x, y + h * 0.45, 5, 0, Math.PI * 2)
  c.stroke()
  for (let i = 0; i < 8; i++) {
    const a = (i / 8) * Math.PI * 2
    c.fillStyle = 'rgba(255,200,80,0.4)'
    c.beginPath()
    c.arc(x + Math.cos(a) * 10, y + h * 0.45 + Math.sin(a) * 5, 1.2, 0, Math.PI * 2)
    c.fill()
  }
  c.strokeStyle = 'rgba(255,220,160,0.5)'
  c.lineWidth = 1.5
  c.beginPath()
  c.ellipse(x, y, w / 2, h / 4, 0, Math.PI * 1.05, Math.PI * 1.95)
  c.stroke()
  c.restore()
}
...
function checkCatch(l: Leaf): boolean {
  return (
    Math.abs(l.x - bowl.x) < bowl.w / 2 - 4 &&
    l.y > bowl.y - bowl.h / 4 &&
    l.y < bowl.y + bowl.h / 4
  )
}
...
function render(): void {
  ...
  if (screen.value === 'playing') {
    ...
    if (sm > 1.5) {
      ...
      c.font = '11px serif'
      c.textAlign = 'right'
      c.fillText(`×${sm.toFixed(1)} tốc độ`, W - 8, 16)
      c.restore()
    }
  }
}
...
function onTouchMove(e: TouchEvent): void {
  ...
  bowl.x = Math.max(
    bowl.w / 2,
    Math.min(W - bowl.w / 2, (e.touches[0]!.clientX - r.left) * (W / r.width)),
  )
}
...
.g-btn:active {
  background: rgba(220, 150, 50, 0.55);
}
