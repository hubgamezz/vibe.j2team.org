...
export function P(type: PieceType, color: PieceColor): Piece { return { type, color } }
...
export function cloneBoard(b: Board): Board { return b.map(row => row.map(p => p ? { ...p } : null)) }

export function inBounds(r: number, c: number): boolean { return r >= 0 && r <= 9 && c >= 0 && c <= 8 }
